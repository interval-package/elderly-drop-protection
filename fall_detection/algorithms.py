...
def extract_keypoints_parallel(queue, args, self_counter, other_counter, consecutive_frames, event, vis_info):
    try:
        # cam, tagged_df = get_source(args)
        if vis_info == 'integrate':
            cam = cv2.VideoCapture(0)
        else:
            cam = cv2.VideoCapture(vis_info)
        tagged_df = None
...
def alg2_sequential(queues, argss, consecutive_frames, event, counter):
    ...
    while True:
        ...
        if not any(q.empty() for q in queues):
            ...
            if argss[0].num_cams == 1:
                ...
                img_dict = dict_frames[0]
                tagged_df = img_dict["tagged_df"]
                # prediction 为预测结果
                counter.value = prediction + 5
                # print(img1.shape)
